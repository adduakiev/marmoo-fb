...
function phoneMatches(raw){return String(raw||'').match(/(?:\+?38)?0\d{9}/g)||[]}
function normalizePhone(raw){const matches=phoneMatches(raw);if(!matches.length)return'';let d=matches[0].replace(/\D/g,'');if(d.length===10&&d.startsWith('0'))d=`38${d}`;return d.length===12&&d.startsWith('380')?d:''}
...
const zeroRevenueOrders=orders.filter(o=>o.order_id&&number(o.order_revenue)===0).length;
const duplicateOrderIds=validOrders.length-new Set(validOrders.map(o=>o.order_id)).size;
...
const customerMap=new Map();let latest='';let identifiedOrders=0;
for(const o of validOrders){const date=isoDate(o.sale_date);if(date>latest)latest=date;const phone=normalizePhone(o.customer_phone||o.customer_phone_raw);if(!phone)continue;identifiedOrders++;const cur=customerMap.get(phone)||{phone,ordersCount:0,totalSpend:0,firstOrderDate:date,lastOrderDate:date,channels:new Set()};cur.ordersCount++;cur.totalSpend+=number(o.order_revenue);if(date<cur.firstOrderDate)cur.firstOrderDate=date;if(date>cur.lastOrderDate)cur.lastOrderDate=date;if(o.channel)cur.channels.add(o.channel);customerMap.set(phone,cur)}
const reference=latest?new Date(`${latest}T00:00:00`):new Date();
const sortedCustomers=[...customerMap.values()].sort((a,b)=>a.phone.localeCompare(b.phone));
const baseProfiles=sortedCustomers.map((c,index)=>({customerId:`Клієнт-${String(index+1).padStart(3,'0')}`,phone:`Клієнт-${String(index+1).padStart(3,'0')}`,ordersCount:c.ordersCount,totalSpend:c.totalSpend,firstOrderDate:c.firstOrderDate,lastOrderDate:c.lastOrderDate,channels:[...c.channels],averageCheck:c.ordersCount?c.totalSpend/c.ordersCount:0,recencyDays:Math.max(0,Math.round((reference-new Date(`${c.lastOrderDate}T00:00:00`))/86400000))}));
