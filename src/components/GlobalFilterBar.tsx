...
export default function GlobalFilterBar({route}:{route:FilterRoute}) {
  ...
  const activeCount=(filters.selectedChannels.length||0)+(filters.compareLFL?1:0)+(filters.normalizeProducts?1:0)+(filters.hideServiceItems?1:0)+(filters.searchQuery?1:0);
  const summary=useMemo(()=>{
    const parts:string[]=[];
    if(config.period)parts.push(periodLabel);
    if(config.channels)parts.push(filters.selectedChannels.length?`${filters.selectedChannels.length} каналів`:'Усі канали');
      ...
      <div className="mx-auto max-w-[1600px]">
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex min-w-0 flex-1 items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[.035] px-3 py-2.5 text-left transition hover:bg-white/[.065]"
          >
            <span className="flex min-w-0 items-center gap-2.5">
              <span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#cfeeed]/10 text-[#cfeeed]"><SlidersHorizontal size={15}/>{activeCount>0&&<span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#cfeeed] px-1 text-[9px] font-black text-[#531027]">{activeCount}</span>}</span>
              <span className="min-w-0">
                <span className="block text-[10px] font-black uppercase tracking-[.15em] text-[#cfeeed]/55">Фільтри</span>
                <span className="block truncate text-xs font-bold text-white/68 sm:text-sm">{summary||'Немає активних фільтрів'}</span>
              </span>
            </span>
        ...
        {open&&<div id="global-filter-panel" className="mt-2 rounded-2xl border border-white/10 bg-black/10 p-3 shadow-[0_18px_45px_rgba(25,0,9,.18)] md:p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-[#cfeeed]/55"><Filter size={14}/>Налаштування звіту</div>
            <button type="button" onClick={resetFilters} className="inline-flex items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-bold text-white/50 hover:bg-white/[.06] hover:text-white"><RotateCcw size={13}/>Скинути</button>
          </div>
          ...
          {hasSecondary&&<div className="grid gap-3 xl:grid-cols-[1fr_auto_auto_auto] xl:items-center">
            ...
            {config.compare&&<label className="flex min-w-max items-center gap-2 rounded-xl border border-white/8 bg-white/[.025] px-3 py-2.5 text-xs font-bold text-white/65"><input type="checkbox" checked={filters.compareLFL} onChange={e=>setFilters(prev=>({...prev,compareLFL:e.target.checked}))}/>Порівнювати з минулим періодом</label>}
            {config.normalize&&<label className="flex min-w-max items-center gap-2 rounded-xl border border-white/8 bg-white/[.025] px-3 py-2.5 text-xs font-bold text-white/65"><input type="checkbox" checked={filters.normalizeProducts} onChange={e=>setFilters(prev=>({...prev,normalizeProducts:e.target.checked}))}/>Об’єднувати дублікати страв</label>}
            {config.service&&<label className="flex min-w-max items-center gap-2 rounded-xl border border-white/8 bg-white/[.025] px-3 py-2.5 text-xs font-bold text-white/65"><input type="checkbox" checked={filters.hideServiceItems} onChange={e=>setFilters(prev=>({...prev,hideServiceItems:e.target.checked}))}/>Приховати технічні позиції</label>}
          </div>}
