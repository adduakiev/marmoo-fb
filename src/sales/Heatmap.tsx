...
export default function Heatmap({cells}:{cells:Cell[]}){
  const{filters,toggleHour,toggleWeekday,toggleHeatCell}=useFilters();
  ...
  return <div className="overflow-x-auto"><div className="min-w-[780px]">
    ...
    <div className="grid gap-1" style={{gridTemplateColumns:`72px repeat(${hours.length},minmax(44px,1fr))`}}>
      <div/>
      {hours.map(hour=>{const active=filters.selectedHours.includes(hour);return <button type="button" key={hour} onClick={()=>toggleHour(hour)} className={`rounded-lg pb-2 pt-1 text-center text-[10px] transition ${active?'bg-[#cfeeed] font-black text-[#531027]':'text-white/40 hover:bg-white/[.06] hover:text-white'}`}>{hour}:00</button>})}
      {DAYS.map((day,index)=>{const weekday=index+1;const dayActive=filters.selectedWeekdays.includes(weekday);return <React.Fragment key={day}>
        <button type="button" onClick={()=>toggleWeekday(weekday)} className={`flex items-center rounded-lg px-2 text-xs font-bold transition ${dayActive?'bg-[#cfeeed] text-[#531027]':'text-white/55 hover:bg-white/[.06] hover:text-white'}`}>{day}</button>
        {hours.map(hour=>{const cell=operatingCells.find(x=>x.weekday===weekday&&x.hour===hour);const intensity=cell?cell.revenue/max:0;const active=filters.selectedWeekdays.includes(weekday)&&filters.selectedHours.includes(hour);return <button type="button" key={hour} onClick={()=>toggleHeatCell(weekday,hour)} title={cell?`${day} ${hour}:00 · ${money(cell.revenue)} · ${cell.orders} чеків`:`${day} ${hour}:00 · немає продажів`} aria-label={cell?`${day} ${hour}:00, ${money(cell.revenue)}, ${cell.orders} чеків`:`${day} ${hour}:00, немає продажів`} className={`h-10 rounded-lg border transition hover:scale-[1.06] hover:border-[#cfeeed]/70 focus:outline-none focus:ring-2 focus:ring-[#cfeeed]/70 ${active?'border-[#f3c969] ring-2 ring-[#f3c969]/80':'border-white/[.05]'}`} style={{background:`rgba(207,238,237,${0.04+intensity*0.86})`}}/>})}
      </React.Fragment>})}
    </div>
  </div></div>
}
